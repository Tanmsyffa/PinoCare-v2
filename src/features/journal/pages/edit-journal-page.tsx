...
import type { Journal } from "@/features/journal/journal-types";
...
export default function EditJournalPage() {
      ...
      .then((data: { journal?: Journal }) => {
        ...
        if (journal) {
          ...
          if (journal.photo_url) {
            setExistingPhotoPath(journal.photo_path || null);
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setExistingPhotoPath(null); // Jika pilih foto baru, hapus reference foto lama
    }
  };
  ...
  const onSubmit = async (data: FormData) => {
    setSaving(true);
    try {
      let photo_path: string | null = existingPhotoPath; // Gunakan foto lama jika tidak diganti
      ...
      if (photo) {
        ...
        if (uploadRes.ok) {
          ...
          if (!uploadData.path) throw new Error("Gagal mengupload foto");
          photo_path = uploadData.path;
        } else {
          throw new Error("Gagal mengupload foto");
        }
      } else if (!photoPreview) {
        // Jika preview dikosongkan (di-hapus oleh user), berarti photo_path harus null
        photo_path = null;
      }
